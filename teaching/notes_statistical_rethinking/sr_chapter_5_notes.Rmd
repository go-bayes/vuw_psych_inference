...
```{r  dag, fig.cap = " A causal graph (DAG) reveals lurking confounders in the the indicators.Were we to include all variables this would lead to collider confounding. means that we must take care when selecting variables for inclusion in our statistical model.", cache = TRUE, echo = FALSE}
# dag
library(tidyverse)
library(ggdag)
dj <- ggdag::dagify(
...
```{r}
stratML <- ggdag::ggdag_drelationship(
  dj,
  from = "cv",
  to = "k6",
